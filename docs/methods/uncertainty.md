...
Processing is chunked to at most 256 realizations per batch, so raising sample count to the supported maximum 8192 does not multiply the dense Jacobian allocation without bound. At the 120-node/240-edge graph cap, its principal float64 batch array is below 266 MiB before solver work buffers. The recorded `batchSize` exposes this execution parameter. Additional samples improve frequency resolution, not empirical calibration of the assumed distribution.
