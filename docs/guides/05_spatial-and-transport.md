...
The **Tool section** selector groups each task into a panel that fits the screen. **View** opens display, level and section controls; **Results** opens the selected result or transport monitor. Focus fills the viewport; press Escape or the focus button to return. On small screens, the tool button opens the same sections over the scene.
...
Select a tunnel in the scene or the **Network tables** dialog. **Selected airway** contains its type, area, resistance, forward-flow target and level; **Junction coordinates** contains the selected junction's exact coordinates and pressure boundary. **Split** makes a real intermediate junction and conserves the entered series resistance; a fan split retains one fan. **Fan** equips the selected tunnel, with its editable curve in **Selected equipment** under an operating mode. The visual width multiplier, level separation, camera and cut affect presentation only. Physical area affects velocity and tracer volume; resistance remains an independent engineering input.
...
3. In **Ventilation change**, optionally schedule a common fan-speed change and airway closure. This assumes immediate equilibrium airflow at the scheduled time, with tracer mass retained.
4. Select **Simulate transport**. Failed or over-budget runs show the actual reason. Running work can be cancelled.
5. **Playback and monitor** opens automatically: play, pause or scrub calculated time. Tunnel-cell colors use one scale over the whole run. Pick a tunnel to link its volume-averaged concentration history, then click the plot to select a calculated frame. A failed run displays its actual error here with a direct link to edit the release.
6. Open **Mass balance and export** to inspect injected, stored, escaped, removed and balance-error mass. **Export time series** retains the network, initial options, exact transport request and every output frame.
...
**Sección de herramientas** agrupa los controles sin una barra lateral larga. **Vista** abre las opciones visuales y el corte; **Resultados** abre el resultado seleccionado o el monitor. La vista enfocada ocupa la pantalla; Escape permite volver. En pantallas pequeñas, el botón de herramientas abre las mismas secciones sobre la escena. **Condiciones operativas** y **Equipo seleccionado** separan los parámetros globales de los de la galería.
