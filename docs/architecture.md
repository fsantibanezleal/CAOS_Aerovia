...
`workbench/Workbench.tsx` composes the dominant scene and one contextual operating area. `ToolPages.tsx` switches explicitly named task sections while keeping inactive content mounted. Linked plots, playback and result inspection occupy sections in the same area; no permanent lower dock reduces the canvas. Focus fills the viewport and provides an Escape exit; mobile controls use an overlay. `workbench/useWorkbench.ts` owns active network/options, result association, history, baseline and selection. `ProjectDialog.tsx` handles project and mapped-table input. Result identity is checked against the active physical input before dependent rendering; an old prediction cannot be drawn onto a newly imported graph.
